...
if (empty($_GET['data'])) {
                    if($date >= $bdate and $date <= $edate){
                        ...
                        $date_start = "$Y-10-01";
                        $date_end = "$y-09-30";
$sql="select count(t1.takerisk_id) as number_risk,r1.level_risk,r1.num from takerisk t1
                                 INNER JOIN level_risk r1 on r1.num = t1.LR_id
 				 where t1.move_status='N' and t1.take_date between '$date_start' and '$date_end' 
				 group by t1.LR_id order by r1.num asc";
 $conn_DB->imp_sql($sql);
